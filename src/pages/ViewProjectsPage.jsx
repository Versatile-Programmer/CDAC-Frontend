...
// import PaginationControls from "../components/PaginationControls"; // Required for pagination
...
// import { FaFileSignature, FiLoader } from "react-icons/fa"; // Added icons
...
//   // Data fetching logic wrapped in useCallback and updated for pagination
...
//           },
//           params: { // Added params for pagination
//             page: page,
//             size: pageSize,
//           },
//         }
...
//       // Destructure paginated data from the response
...
//   // Map over the current page's projects to add a serial number and action button.
//   const dataWithActions = projects.map((item, index) => ({
//     ...item,
//     serialNo: page * pageSize + index + 1, // Correct calculation for pagination
...
// import { useRecoilValue } from "recoil";
// import { authTokenState } from "../recoil/atoms/authState.js";
// import { FaFileSignature } from "react-icons/fa"; // Icon for the apply button
// import { FiLoader } from "react-icons/fi"; // Icon for loading state
...
//   const navigate = useNavigate();
//   const empId = user?.id;
//   const currentTheme = getThemeForDays("default"); // Get a default theme for the table

//   // Simplified state without pagination
//   const [projects, setProjects] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState(null);
...
//   // Function to navigate to the domain application form
//   const handleApplyForDomain = (projectId) => {
//     navigate(`/domains/add?projectId=${projectId}`);
//   };
...
//       setError(null);
//       try {
//         // The API call is now simpler, without pagination params
//         const response = await axios.get(
//           `${API_BASE_URL}/api/users/list/projects/${empId}`,
//           {
//             headers: {
...
//     fetchProjects();
//   }, [empId, authToken]); // Dependencies are simpler now

//   // Map over the fetched projects to add a serial number and an action button.
//   const dataWithActions = projects.map((item, index) => ({
//     ...item,
//     serialNo: index + 1, // Simple serial number for a non-paginated list
//     project_name: <span className="font-medium text-gray-800">{item.project_name}</span>,
//     actions: (
//       <button
//         onClick={() => handleApplyForDomain(item.project_id)}
...
function DrmProjectsPage() {
  ...
  // Map over the fetched projects to add styled elements and apply text wrapping
  const dataWithActions = projects.map((item, index) => ({
    ...item,
    serialNo: index + 1,
    project_name: <span className="font-medium text-gray-800">{item.project_name}</span>,

    // --- MODIFIED: Added a container with classes for robust text wrapping ---
    project_remarks: (
